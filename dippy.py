...
# 		# convert image to triple channel np format
# 		img_cv2 = cv2.cvtColor(np.array(image), cv2.COLOR_RGB2BGR)
# 		img_cv2 = img_cv2.astype(np.uint16)
# 		img_cv2[:,:,2] += value  # add value to red channel
# 		img_cv2[img_cv2 > 255] = 255  # overflow correction
# 		img_cv2 = img_cv2.astype(np.uint8)
# 		temp_image = Image.fromarray(cv2.cvtColor(img_cv2, cv2.COLOR_BGR2RGB))
# 		temp_image_tk = ImageTk.PhotoImage(temp_image)
# 		canvas.delete("all")
# 		canvas.config(width=temp_image.width, height=temp_image.height)
# 		canvas.create_image(0, 0, anchor=tk.NW, image=temp_image_tk)
